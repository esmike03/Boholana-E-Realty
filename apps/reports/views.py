...
# ── Standard date/time formatting (project-wide) ──────────────────────
# USE_TZ=True with TIME_ZONE='Asia/Manila': always convert to local time
# before formatting, otherwise timestamps render in UTC.
STD_DATETIME_FMT = '%b %d, %Y %I:%M %p'
STD_DATE_FMT = '%b %d, %Y'


def fmt_dt(value):
    """Format a datetime in local (Manila) time using the standard format."""
    if not value:
        return 'N/A'
    try:
        ...
        pass
    return value.strftime(STD_DATETIME_FMT)


def fmt_date(value):
    if not value:
        return 'N/A'
    return value.strftime(STD_DATE_FMT)
        ...
        for sa in sale_assistants:
            ...
            for sale in sa_sales.order_by('-sale_date'):
                ...
                if disbursement:
                    ...
                    paid_at = fmt_dt(disbursement.disbursed_at)
                else:
...
def generate_pdf_report(report, data):
    ...
    doc = SimpleDocTemplate(
        buffer, pagesize=A4,
        topMargin=48, bottomMargin=40, leftMargin=40, rightMargin=40,
    )
    styles = getSampleStyleSheet()
    elements = []

    # ── Branded report header ─────────────────────────────────────────
    brand_style = styles['Title'].clone('BrandTitle')
    brand_style.fontSize = 20
    brand_style.textColor = colors.HexColor('#1a4a7a')
    brand_style.spaceAfter = 2
    tagline_style = styles['Normal'].clone('Tagline')
    tagline_style.fontSize = 8
    tagline_style.textColor = colors.HexColor('#8a8a8a')
    subtitle_style = styles['Heading2'].clone('ReportSubtitle')
    ...
    meta_style.textColor = colors.HexColor('#555555')

    elements.append(Paragraph("BOHOLANA E-REALTY", brand_style))
    elements.append(Paragraph("Property &amp; Legal Services — Tagbilaran City, Bohol", tagline_style))
    elements.append(Spacer(1, 4))
    # Divider line
    divider = Table([['']], colWidths=[515], rowHeights=[2])
    divider.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, -1), colors.HexColor('#c9a24b')),
    ]))
    elements.append(divider)
    elements.append(Spacer(1, 10))

    # Report title
    elements.append(Paragraph(f"<b>{report.get_report_type_display()}</b>", subtitle_style))
    elements.append(Spacer(1, 4))

    # Meta: period, generated timestamp, generated by
    if report.date_from or report.date_to:
        elements.append(Paragraph(
            f"Period: {fmt_date(report.date_from)} to {fmt_date(report.date_to)}",
            meta_style
        ))
    generated_by = report.generated_by.get_full_name() if report.generated_by else 'System'
    elements.append(Paragraph(f"Generated by: {generated_by}", meta_style))
    elements.append(Paragraph(
        f"Generated on: {fmt_dt(timezone.now())}",
        meta_style
    ))
...
def transaction_list(request):
    """Admin read-only view of all financial transactions (sales + payments).

    This is a view-only oversight page — no status editing or workflow actions.
    """
    from django.core.paginator import Paginator
    from apps.sales.models import PaymentSchedule

    user = request.user
    if user.role != 'admin' and not user.is_superuser:
        messages.error(request, 'Only administrators can view transactions.')
        return redirect('dashboard')

    sales = Sale.objects.select_related('property', 'client').order_by('-created_at')
    ...
    # Annotate each sale with total paid + balance
    rows = []
    grand_net = grand_paid = 0
    for sale in sales:
        paid = PaymentSchedule.objects.filter(sale=sale).aggregate(t=Sum('amount_paid'))['t'] or 0
        grand_net += sale.net_price
        grand_paid += paid
        rows.append({
            'sale': sale,
            'paid': paid,
            'balance': sale.net_price - paid,
        })
    ...
    return render(request, 'reports/transactions.html', {
        ...
        'grand_balance': grand_net - grand_paid,
    })
...
def report_create(request):
    ...
    if request.method == 'POST':
        ...
        # Excel/CSV export removed — reports are PDF only.
        format_type = 'pdf'
